...
def _wait_for_profile_page(browser_runtime, target_id, timeout_seconds=20, interval_seconds=0.5):
    started = time.time()
    last_page = None
    while time.time() - started < timeout_seconds:
        page = browser_runtime.get_page_info(target_id)
        last_page = page
        if page and _is_profile_url(page.get("url")):
            return page
...
def run(read_service, browser_runtime, target_id=None, params=None, timeout_seconds=20):
    ...
    try:
        final_page = _wait_for_profile_page(
            browser_runtime=browser_runtime,
            target_id=resolved_target_id,
            timeout_seconds=min(timeout_seconds, 20),
            interval_seconds=0.5,
        )
        if not final_page or not _is_profile_url((final_page or {}).get("url")):
            return {
                "ok": False,
                "site": "xiaohongshu",
                "workflow": "read_profile_metrics",
                "targetId": None if not opened.get("reused") else resolved_target_id,
                "error": "failed to resolve final profile url",
                "page": final_page or {},
            }
        ...
        return {
            ...
            "items": (read_result.get("content") or {}).get("recentPosts") or [],
            "checkpoint": {},
            "page": read_result.get("page") or {},
