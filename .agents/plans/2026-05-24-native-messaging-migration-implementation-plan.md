...
**Steps：**
- [ ] 创建分支：`git checkout -b feature/native-messaging`
- [ ] 创建 `bridge/app/native_messaging.py`，实现两个函数：
...
- [ ] 编译检查：`env PYTHONPYCACHEPREFIX=/tmp/browser-bridge-pycache python3 -m py_compile bridge/app/native_messaging.py`
...
**Steps：**
- [ ] 创建 `bridge/app/native_session_manager.py`，实现 `NativeSessionManager` 类：
...
- [ ] 在 `server.py` 中注入 NativeSessionManager：
...
- [ ] 编译检查：`env PYTHONPYCACHEPREFIX=/tmp/browser-bridge-pycache python3 -m py_compile bridge/app/native_session_manager.py bridge/app/server.py`
...
**Steps：**
- [ ] 创建 `bridge/app/native_host_shim.py`，使用单 event loop + asyncio.to_thread 模型（避免跨线程/loop 使用 WebSocket 对象）：
  ```python
  #!/usr/bin/env python3
  ...
  if __name__ == '__main__':
      asyncio.run(run())
  ```
- [ ] 确保文件可执行：`chmod +x bridge/app/native_host_shim.py`
- [ ] 编译检查：`env PYTHONPYCACHEPREFIX=/tmp/browser-bridge-pycache python3 -m py_compile bridge/app/native_host_shim.py`
...
- [ ] 创建 `native-messaging/com.cuiguidong.browserbridge.json`：
  ```json
  {
...
- [ ] 创建 `scripts/install-native-host.sh`：
  - 检测操作系统（Linux / macOS）
  - 检测浏览器（Chrome / Edge）
...
**Steps：**
- [ ] 在 `manifest.json` 的 `permissions` 数组中添加 `"debugger"` 和 `"nativeMessaging"`
- [ ] 在 `background.js` 顶部新增 Native Messaging 连接代码（**保留现有 HTTP 轮询逻辑不动**）：
  ```javascript
  ...
  ```
- [ ] 同步扩展到宿主机：`cp extension/manifest.json extension/background.js /Users/cuiguidong/workspace/personal/projects/browser-bridge-project/extension/`
- [ ] 在 `edge://extensions` 中重载扩展
...
**Steps：**
- [ ] 确保 bridge daemon 已重启（包含 /native/ws endpoint）
- [ ] 确保 native host manifest 已安装
- [ ] 确保扩展已重载且 connectNative 成功（edge://extensions 的 Service Worker 控制台无 `connectNative failed` 错误）
- [ ] 在 `server.py` 新增两个调试 endpoint（仅用于开发调试，阶段 5 清理时移除）：
...
**Steps：**
- [ ] 创建 `NativeBrowserRuntime` 类，构造函数接收 `NativeSessionManager` 实例
- [ ] 实现 15 个方法，每个方法通过 `session_manager.send_command()` 调用扩展：
...
- [ ] 实现统一 helper `_resolve_native_tab_id(target_id=None)`：
...
- [ ] 编译检查：`env PYTHONPYCACHEPREFIX=/tmp/browser-bridge-pycache python3 -m py_compile bridge/app/native_browser_runtime.py`
...
**Steps：**
- [ ] 在 `handleNativeCommand` 中添加完整命令分发（替换 Task 1.5 的 placeholder）：
...
- [ ] 添加 `ensureDebuggerAttached(tabId)` 函数（含 attachedTabs cache 和错误分类）
- [ ] 添加 `handleDebuggerCommand(method, params)` 函数
- [ ] 添加 `chrome.tabs.onRemoved` 监听器清理 attach cache
- [ ] 同步到宿主机并重载扩展
...
- [ ] 在 `config.py` 新增：
  ...
  ```
- [ ] 在 `cdp_runtime.py` 的 `CdpRuntime` 类中：
...
- [ ] 在 `server.py` 中，将 `native_session_manager` 注入到 `CdpRuntime` 构造函数
- [ ] 编译检查
...
**Steps：**
- [ ] 确认浏览器**未**开 `--remote-debugging-port`，正常启动 Edge
- [ ] 通过 systemd drop-in 或 `.env.local` 设置 `BROWSER_RUNTIME=native_only`，重启 bridge daemon：
  ```bash
  sudo systemctl restart browser-bridge.service
  curl --noproxy '*' -sS http://127.0.0.1:17777/health
  ```
- [ ] 逐个验证（通过 curl 或 Python 脚本）：
...
**Steps：**
- [ ] 确保 `BROWSER_RUNTIME=native_only`，浏览器未开 --remote-debugging-port
- [ ] 运行：`python3 skills/weibo-assistant/scripts/read_post.py 'https://weibo.com/6105713761/Qy80W8wXc'`
- [ ] 检查返回 ok:true，包含 author/text/images 字段
...
**Steps：**
- [ ] 运行：`python3 skills/x-assistant/scripts/read_post.py 'https://x.com/billtheinvestor/status/2038173185875775987'`
- [ ] 运行：`python3 skills/x-assistant/scripts/search.py "AI agent"`
- [ ] 检查两个结果都返回 ok:true
...
**Steps：**
- [ ] 运行：`python3 skills/xiaohongshu-assistant/scripts/read_post.py 'https://www.xiaohongshu.com/explore/69c6469e000000001d01d9d1'`
- [ ] 检查返回 ok:true
- [ ] 测试文件上传（prepare_publish_post）：
  - 准备一张测试图片路径
  - 通过 workflow API 调用 prepare_publish_post
  - 验证图片上传成功，页面停在发布按钮前
  - **重点检查**：`DOM.setFileInputFiles` 通过 chrome.debugger 是否正常工作
- [ ] 如果文件上传失败，检查 chrome.debugger 对 DOM 命令的支持情况
...
**Steps：**
- [ ] 运行：`python3 skills/zhihu-assistant/scripts/read_post.py 'https://zhuanlan.zhihu.com/p/1899044076168394425'`
- [ ] 运行：`python3 skills/zhihu-assistant/scripts/read_hot.py`
- [ ] 检查两个结果都返回 ok:true
...
**Steps：**
- [ ] 修改 `ExtensionRuntime.__init__()` 接收 `native_session_manager=None` 和 `site_registry=None` 参数并存储
- [ ] 扩展 `invoke()` 签名为 `invoke(method, params, timeout_seconds=30, target_url=None, target_id=None, site=None)`
- [ ] 在 `_resolve_semantic_tab_id()` 的第 3 优先级（无 target_id 无 target_url）中，通过 `self._site_registry.get(site).hosts` 获取站点 hosts 列表，用于过滤匹配站点 tab
- [ ] 在 `server.py` 中将 `native_session_manager` 和 `site_registry` 注入到 `ExtensionRuntime` 构造函数
- [ ] 在 `extension_runtime.py` 中新增 `_resolve_semantic_tab_id(target_id=None, target_url=None, site=None)` 方法：
...
- [ ] 在 `invoke()` 中：如果有活跃 native session，调用 `_resolve_semantic_tab_id(target_id, target_url, site)` 解析 tabId，解析失败返回 `{ok: false, error: "tab_not_found"}`，不发命令给扩展；解析成功通过 `native_session_manager.send_command()` 发送 `{method, params, tabId: <整数>}`
- [ ] 在 `server.py` 中将 `native_session_manager` 注入到 `ExtensionRuntime` 构造函数
- [ ] 更新 `read_service.py` 和 `action_service.py` 中所有 `extension_runtime.invoke()` 调用，传入 `target_id` 和 `site` 参数（从 workflow 传入的上下文中获取）
- [ ] 在 `background.js` 的 `handleNativeCommand` 中实现 `semantic.invoke`：
...
**Steps：**
- [ ] content.js 的 `reportSnapshot()` 不改动——它仍通过 `chrome.runtime.sendMessage({action: 'extensionSnapshot', payload})` 发给 background.js
- [ ] background.js 的 `postReport(payload)` 函数改为：
  - 优先通过 `nativePort.postMessage({type: "report", payload})` 发送（native 通道）
  - `nativePort` 不可用时回退到 HTTP POST `/extension/report`
- [ ] 在 NativeSessionManager 中处理 report 消息：
  - 收到 `{type: "report", payload: {...}}` 时存入 `_report_cache`
  - 暴露 `get_report(session_id)` 方法供 ExtensionRuntime 查询
- [ ] 在 ExtensionRuntime 中：
...
**Steps：**
- [ ] 在 `BROWSER_RUNTIME=native_only` 下重跑 Task 3.1-3.4 的全部验证
- [ ] 额外测试 X 的 follow_user / bookmark 动作（如方便的话）
...
**Steps：**
- [ ] 删除三个 CDP 文件
- [ ] 从 `config.py` 移除 `CDP_PUBLIC_HOST`、`CDP_CONNECT_HOST`、`CDP_PORT`、`CDP_BASE_URL`、`CDP_CONNECT_BASE_URL`、`CDP_HOST_HEADER`、`CDP_WS_BASE_URL`、`CDP_TIMEOUT_SECONDS`
- [ ] 从 `cdp_runtime.py` 移除 CDP 分支代码，`CdpRuntime` 直接委托给 `NativeBrowserRuntime`
- [ ] 从 `server.py` 移除 CDP 相关导入
- [ ] 从 `server.py` 移除 `/native/debug/ping` 和 `/native/debug/tabs` 调试 endpoint
- [ ] 编译检查
...
**Steps：**
- [ ] 从 server.py 移除 `/extension/pull`、`/extension/result`、`/extension/report` 端点（全部 HTTP 轮询端点一起移除，因为阶段 4 已确认 report 走 native）
- [ ] 从 background.js 移除 HTTP 轮询相关函数（pullBridgeCommand、postBridgeCommandResult、pollDevCommandOnce）；将 `postReport(payload)` 重命名为 `sendNativeReport(payload)`，只保留 `nativePort.postMessage({type: "report", payload})` 路径，移除 HTTP fallback；保留 `extensionSnapshot` 的 message handler（content.js → background.js 入口不变）
- [ ] 从 manifest.json 移除 `"host_permissions": ["http://127.0.0.1:17777/*"]`（此时扩展已不依赖 HTTP 访问 bridge）
- [ ] 同步扩展并重载
- [ ] 全量回归验证
...
**Steps：**
- [ ] README.md：快速开始部分移除 --remote-debugging-port 启动步骤，改为"安装 native host manifest + 启动 bridge"
- [ ] docs/development.md：安装与启动部分更新，移除 CDP 端口配置，新增 native host 安装步骤
- [ ] docs/operations.md：环境变量表移除 CDP_* 变量，新增 BROWSER_RUNTIME 变量说明
- [ ] 更新 AGENTS.md 中对 CDP 的引用（如有）
- [ ] 更新 LOCAL_DEV.md 中对 CDP 端口的引用（如有）
- [ ] 更新 docs/capabilities.md 中的相关说明（如有）
...
**Steps：**
- [ ] 最终全量回归验证（`BROWSER_RUNTIME=native_only`，无 --remote-debugging-port）：
  - 微博 read_post
  - X read_post + search
  - 小红书 read_post
...
- [ ] **等待用户明确批准后**才执行合并
- [ ] 合并后更新 .agents/task-board.yaml 记录迁移完成
