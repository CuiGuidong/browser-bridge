...
**实现逻辑：**
为了解决大模型查看图片需要消耗大量时间同步下载或 Token (Base64) 的问题，系统采用了“文件系统级别的异步 Promise”模式：
1. **正则替换与占位**：Python 脚本在返回数据前，利用计算决定论，通过对在线图片 URL 进行 MD5 哈希，推算出确定性的本地绝对路径。并将原文的 `[Image: URL]` 替换为 `[Image Local: <path> | Remote: <url>]` 立即返回给大模型。
2. **分离拉起下载器**：主脚本利用 `subprocess.Popen` 无阻塞地拉起 `async_image_downloader.py` 后台幽灵进程，并发下载图片至本地缓存目录。
3. **时间差互补**：当大模型阅读 JSON 并思考几秒钟后，如果它决定调用 `read_file` 工具查看图片，此时后台下载必定已经完成。这实现了对大模型完全透明、零延迟的多模态极速闭环。
