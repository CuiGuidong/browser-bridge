...
## 6. 待研究事项 (Pending Research)

### 图片 Base64 内联与原生提取
**设想**：绕过系统下载，直接在浏览器中通过 Canvas 或在 Python 内存中提取图片的 Base64 数据返回。
**阻碍**：浏览器端存在跨域 (CORS) 画布污染问题；Python 内存转 Base64 虽可行，但会导致返回的 JSON 极度膨胀（Token 消耗过大），破坏大模型上下文窗口。需继续寻找兼顾效率与 Token 占用的多模态喂图方案。
