...
- current state: scaffolding exists, but network signal is not yet reliably consumed in final adapter output
...
- X adapter can extract primary content, but still carries some timeline / metadata noise
- request probe state exists architecturally but is not yet reflected reliably in adapter reports (`signals.network` can remain null)
